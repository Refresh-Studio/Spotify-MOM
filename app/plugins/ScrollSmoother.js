  ...
  constructor(vars) {
    ...
    let { smoothTouch, onUpdate, onStop, smooth, onFocusIn, normalizeScroll, wholePixels } = vars,
      ...
      adjustParallaxPosition = (triggers, createdAfterEffectWasApplied) => {
        effects.forEach((st) =>
          adjustEffectRelatedTriggers(st, triggers, createdAfterEffectWasApplied)
        );
      },
      onRefresh = () => {
        removeScroll();
    ...
    this.scrollTo = (target, smooth, position) => {
      let p = gsap.utils.clamp(
        0,
        _maxScroll(),
        isNaN(target) ? this.offset(target, position, !!smooth && !paused) : +target
      );
      !smooth
        ? scrollTop(p)
        : paused
    ...
    vars.effects &&
      this.effects(
        vars.effects === true
          ...
          : vars.effects,
        { effectsPadding: vars.effectsPadding, refresh: false }
      );
      ...
      if (gsap) {
        ...
        if (_body && ScrollTrigger) {
          _onResizeDelayedCall = gsap
            .delayedCall(
              0.2,
              () => ScrollTrigger.isRefreshing || (_mainInstance && _mainInstance.refresh())
            )
            .pause();
