...
let _id = 1,
  ...
  _makeCompsAbsolute = (comps) =>
    _orderByDOMDepth(comps, true).forEach(
      (c) => (c.a.isVisible || c.b.isVisible) && _makeAbsolute(c.sd < 0 ? c.b : c.a, c.b, 1)
    ),
  ...
  _fromTo = (fromState, toState, vars, relative) => {
    ...
    for (p in toState.idLookup) {
      ...
      if (fromNode) {
        ...
        if (comp.sd) {
          ...
          fade &&
            comps.push(
              (comp.swap = { t: fromNode.element, b: comp.b, a: comp.a, sd: -comp.sd, swap: comp })
            );
        }
    ...
    run = () => {
      ...
      if (targets.length || leaving.length || entering.length) {
        toggleClass &&
          animation.add(
            () => _toggleClass(classTargets, toggleClass, animation._zTime < 0 ? 'remove' : 'add'),
            0
          ) &&
          !paused &&
    ...
    absoluteOnLeave &&
      (absolute = comps
        .filter((comp) => !comp.sd && !comp.a.isVisible && comp.b.isVisible)
        .map((comp) => comp.a.element));
    if (_batch) {
  ...
  compare(state) {
    ...
    for (p in l1) {
      ...
      if (s2Alt) {
        ...
        c1 =
          s1Alt && !s1.isVisible && !s1Alt.isVisible && c2.element === s1Alt.element ? s1Alt : s1;
...
export class Flip {
  ...
  static killFlipsOf(targets, complete) {
    (targets instanceof FlipState ? targets.targets : _toArray(targets)).forEach(
      (t) => t && _killFlip(t._flip, complete !== false ? 1 : 2)
    );
  }
