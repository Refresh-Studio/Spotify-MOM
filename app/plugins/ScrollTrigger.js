...
let gsap,
  ...
  _endAnimation = (animation, reversed, pause) =>
    animation && animation.progress(reversed ? 0 : 1) && pause && animation.pause(),
  _callback = (self, func) => {
    if (self.enabled) {
  ...
  _snapDirectional = (snapIncrementOrArray) => {
    let snap = gsap.utils.snap(snapIncrementOrArray),
      a =
        Array.isArray(snapIncrementOrArray) && snapIncrementOrArray.slice(0).sort((a, b) => a - b);
    return a
  ...
  _addListener = (element, type, func, nonPassive, capture) =>
    element.addEventListener(type, func, { passive: !nonPassive, capture: !!capture }),
  ...
  _clearScrollMemory = (scrollRestoration, force) => {
    ...
    (force || !_refreshingAll) &&
      _scrollers.forEach((obj) => _isFunction(obj) && obj.cacheID++ && (obj.rec = 0));
...
export class ScrollTrigger {
  ...
  init(vars, animation) {
    ...
    self.getTrailing = (name) => {
      ...
      return (_isString(name) ? a.filter((t) => t.vars.preventOverlaps === name) : a).filter((t) =>
        self.direction > 0 ? t.end <= start : t.start >= end
      );
    };
      ...
      if (clipped !== prevProgress && self.enabled) {
        ...
        preventOverlaps &&
          ...
          (_isFunction(preventOverlaps)
            ...
            : self.getTrailing(preventOverlaps).forEach((t) => t.endAnimation()));
...
ScrollTrigger.refresh = (safe) =>
  safe ? _onResize() : (_coreInitted || ScrollTrigger.register()) && _refreshAll(true);
...
let _clampScrollAndGetDurationMultiplier = (scrollFunc, current, end, max) => {
  ...
  _inputObserver = (target, type, inputs, nested) =>
    Observer.create({
      ...
      onScroll: nested,
      onEnable: () =>
        inputs && _addListener(_doc, Observer.eventTypes[0], _captureInputs, false, true),
  ...
  _getScrollNormalizer = (vars) => {
    ...
    tween = gsap.to(self, {
      ...
      scrollY: '+=0.1',
      modifiers: { scrollY: _interruptionTracker(scrollFuncY, scrollFuncY(), () => tween.pause()) },
      onUpdate: _updateAll,
