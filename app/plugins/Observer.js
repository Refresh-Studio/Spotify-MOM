...
let gsap,
  ...
  _addListener = (element, type, func, passive, capture) =>
    element.addEventListener(type, func, { passive: passive !== false, capture: !!capture }),
...
export class Observer {
  ...
  init(vars) {
    ...
    let id,
      ...
      scrollY = scrollFuncY(),
      limitToTouch =
        ~type.indexOf('touch') && !~type.indexOf('pointer') && _eventTypes[0] === 'pointerdown', // for devices that accommodate mouse events and touch events, we need to distinguish.
      ...
      _ignoreCheck = (e, isPointerOrTouch) =>
      ...
      _onRelease = (self.onRelease = (e) => {
        ...
        let isTrackingDrag = !isNaN(self.y - self.startY),
          ...
          isDragNotClick =
            wasDragging &&
            (Math.abs(self.x - self.startX) > 3 || Math.abs(self.y - self.startY) > 3), // some touch devices need some wiggle room in terms of sensing clicks - the finger may move a few pixels.
    ...
    self.disable = () => {
      if (self.isEnabled) {
        ...
        _observers.filter((o) => o !== self && _isViewport(o.target)).length ||
